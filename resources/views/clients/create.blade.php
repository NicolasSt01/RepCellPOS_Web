...
<div class="space-y-6">
    <div>
        <h1 class="text-2xl font-bold text-gray-900 dark:text-gray-100">Nuevo Cliente</h1>
        <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">Registrar un nuevo cliente en el sistema</p>
    </div>
        ...
        <form method="POST" action="{{ route('clients.store') }}" class="p-6 space-y-6">
            ...
            <div class="grid grid-cols-1 gap-6 sm:grid-cols-2">
                ...
                <div>
                    ...
                    <input type="text" name="phone" id="phone" value="{{ old('phone') }}" required placeholder="Ej: +52 55 1234 5678"
                        class="mt-1 block w-full rounded-md border-0 py-1.5 px-3 text-gray-900 dark:text-gray-100 shadow-sm ring-1 ring-inset ring-gray-300 dark:ring-gray-600 focus:ring-2 focus:ring-inset focus:ring-indigo-600 dark:bg-gray-700 sm:text-sm sm:leading-6">
                ...
                <div>
                    ...
                    <select name="notification_preference" id="notification_preference" required
                        class="mt-1 block w-full rounded-md border-0 py-1.5 px-3 text-gray-900 dark:text-gray-100 shadow-sm ring-1 ring-inset ring-gray-300 dark:ring-gray-600 focus:ring-2 focus:ring-inset focus:ring-indigo-600 dark:bg-gray-700 sm:text-sm sm:leading-6">
                        <option value="whatsapp" {{ old('notification_preference') === 'whatsapp' ? 'selected' : '' }}>WhatsApp</option>
                        <option value="email" {{ old('notification_preference') === 'email' ? 'selected' : '' }}>Correo electrónico</option>
                        <option value="call" {{ old('notification_preference') === 'call' ? 'selected' : '' }}>Llamada telefónica</option>
                    </select>
