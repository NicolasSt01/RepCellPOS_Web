...
@endphp
@if($saleClient)
    <p style="font-size:10px;margin:4px 0 0 0;">Cliente: {{ $saleClient->name }}@if($saleClient->phone) — Tel: {{ $saleClient->phone }}@endif</p>
@endif
...
@if(($clauses ?? null) && count($clauses) > 0)
    ...
    @foreach($clauses as $clause)
        @if($clause->has_file)
            <p style="font-size:9px;margin:2px 0;text-align:center;"><a href="{{ $clause->file_url }}" target="_blank" style="color:#2563eb;text-decoration:underline;">Ver política: {{ $clause->file_name }}</a></p>
        @else
            <p style="font-size:9px;margin:2px 0;text-align:center;">{{ $clause->content }}</p>
        @endif
    @endforeach
@endif
